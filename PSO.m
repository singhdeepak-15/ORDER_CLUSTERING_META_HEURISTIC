...
function [man ,st,gbest_pos] = PSO(Np, Nd, Nt, xMin, xMax, vMin, vMax,hdata,hy)
...
while ( r <= run)
    if(nargin <= 9) 
        for i = 1 : 2*Np
            for j = 1:Nd
                if rand > 0.5
                    pop(i,j) = ceil(xMin + (xMax-xMin) * rand);
                else
                    pop(i,j) = ceil(xMin + (xMax-xMin) * rand);
                end
            end
      ...
      flag = 0;
        ...
        if hy == 1
                ...
                size(ch1);
                size(ch2);
                f1 = fitness(ch1,hdata);
                f2 = fitness(ch2,hdata);
                if f1 < M(ceil(fs/2))
                    pop(fs,:) = ch1(1,:);
                    pop(fs+1,:) = ch1(2,:);
                else
